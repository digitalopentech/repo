...
FROM cco
LEFT JOIN eb ON cco.idArchSagaOperation = eb.idArchSagaOperation
             AND cco.cdColigada = eb.cdColigada
             AND cco.cdAgencia = eb.cdAgencia
             AND CAST(cco.nuConta AS BIGINT) = CAST(eb.nuConta AS BIGINT)
LEFT JOIN cc ON cco.idArchSagaOperation = cc.idArchSagaOperation
...
WHERE cco.flContaMigrada = 'S'
  AND cco.movimento_dataContabil_ts IS NOT NULL
  AND cco.ingestionTime IS NOT NULL
  AND eb.ingestionTime IS NOT NULL
  AND cc.ingestionTime IS NOT NULL
